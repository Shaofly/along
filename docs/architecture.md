...
- 账号、会话、朋友关系、邀请、个人资料、动态及媒体元数据都连接 PostgreSQL，并通过 `drizzle/` 中的迁移维护。
...
- 一级页眉统一由共享组件提供。桌面和平板采用“头像 + 首页/圈子/朋友/个人分段导航 / 居中完整 Logo / 通知”，手机采用“头像与主要显示名 / 居中图标 Logo / 通知”；朋友列表和邀请管理必须是独立路由。
