...
- `user_profile_details` 保存性别、规范化现居地、手机号、独立联系邮箱、学校、当前可见范围和上一个非私密范围。现居地仍是一个文本字段，不拆分为省、市数据库列；编辑器按国内或海外结构化收集后写成以` · `分隔的规范字符串，避免固定列无法表达海外地区。当前范围为`private`时，应用层要求`user.nickname`存在，并在个人主页与朋友查询中裁剪`real_name`；这项跨表条件由写入事务和读取防御共同保证，不伪装成单表约束。
