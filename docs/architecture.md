...
- 新图片默认位于 `PRIVATE_DATA_ROOT/media`，旧版 `PRIVATE_DATA_ROOT/uploads` 只为迁移前资源保留兼容读取；本地对应项目根目录下的 `.data`，不提交 Git。
...
- 动态照片统一由 `PhotoViewer` 展示：桌面端使用有边界的居中舞台，移动端使用全屏手势层；媒体地址继续经过现有私有权限接口，查看器不会绕过服务端授权直接读取存储文件。
...
本地或单机服务器应由受控定时任务定期调用媒体维护接口，以继续异常中断的任务并回收过期 incoming 文件：
...
该接口不得暴露给浏览器，也不能使用登录 Cookie 代替独立运维密钥。完整媒体生命周期与正式 COS 方案见 [媒体上传与图片规格](features/media.md)。
