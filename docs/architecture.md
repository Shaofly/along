...
- 小圈子以`circle_member_relations（持久成员关系）`保存不会因退出重算的历史边界，以`circle_membership_periods（成员周期）`记录每次加入与退出，并用`active_period_id（当前活跃成员周期编号）`加速当前权限查询。数据库在事务提交时校验活跃指针与开放周期双向一致。
